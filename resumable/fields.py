...
from widgets import ResumableFileInput, ResumableFileDropInput
...
class ResumableFileDropField(Field):
    default_error_messages = dict(Field.default_error_messages, **{
            'invalid_mime': _(u'Invalid file type')
    })
    widget = ResumableFileDropInput

    def __init__(self, *args, **kwargs):
        self.allowed_mimes = kwargs.pop('allowed_mimes', None)
        upload_url = kwargs.pop('upload_url', None)
        chunks_dir = kwargs.pop('chunks_dir', None)
        super(ResumableFileDropField, self).__init__(*args, label="", **kwargs)
        self.upload_url = upload_url
        self.chunks_dir = chunks_dir
    ...
    def clean(self, data):
        f = super(ResumableFileDropField, self).clean(data)
        if self.allowed_mimes is not None and \
                f.content_type not in self.allowed_mimes:
            raise ValidationError(self.error_messages['invalid_mime'])
        return f
